...
const PendingPostCard = ({ cardDetails, postKey, postTypeComponent }) => {
  ...
  useEffect(() => {
    ...
    setShortTitle(cardDetails?.shortTitle || '')
  ...
  const handleValidation = (type) => {
    ...
    if (cardDetails?.text?.length < 51) {
      setAlert('warning', `You have to enter the minimum 50 characters about post to ${type}`);
      return false;
    }
      if ((cardDetails?.postType === 'onThisDay' || cardDetails?.postType === 'inNews') && (!cardDetails?.shortTitle || cardDetails?.shortTitle === '' || !shortTitle || shortTitle === '')) {
      setAlert('warning', `Please enter the short title before ${type}`);
      return false;
    }
    ...
    if (cardDetails?.postType === 'onThisDay' && !cardDetails?.thisDayDate) {
      setAlert('warning', `Please Enter date of event to ${type}`);
      return false;
    }
      ...
      <Grid container spacing={2} className="pending-post-detail-card">
        ...
        >
          <Grid container spacing={2} className="pending-post-action-div">
            <Grid item md={12} xs={12} style={{ textAlign: 'right', margin:cardDetails?.postType === 'fact' ? '0px 0px 0px 0px' : 0 }}>
                <Grid item md={12} xs={12} style={{ position: 'relative', display: cardDetails?.postType === 'fact' ? 'none' : '', }}>
              <TextField
                    value={shortTitle}
                    maxLength={40}
                    fullWidth
                    ...
                    onBlur={(e) => {
                      e.target.placeholder = 'Enter short title max length 50 characters';
                      if (!shortTitle) {
                ...
                <Grid  md={12} xs={12} style={{ position: 'relative', margin: 0, padding: 0}}>
              <div 
              onClick={() => {
              ...
              style={{
                        textAlign: 'center',
                        color: cardDetails?.segmentType ? '#000' : '#888F9D',
                        ...
                        <h5>
                {cardDetails?.segmentType ? 'Segment type : ' + getSegmentText(cardDetails?.segmentType) : 'Select the Segment group'}
                        </h5>
              </div>
             </Grid>
             <input
