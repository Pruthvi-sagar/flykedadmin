    ...
    <MainContainer bodyHight="90%" footer={
      !isLoading && pendingPostList?.results?.length !== 0 && (
        <Pagination
          ...
          onClickPrevious={() => { 
            setisLoading(true)
            dispatch(pendingPostAction.getPendingUnAssignedStoriesList('pendingPosts', pendingPostList?.prevPage, setAlert, postFilter, () =>  setisLoading(false)));
            dispatch({ type: SELECTED_PENDING_POSTS, payload: [] });
            handleScroll();
          }}
          onClickNext={() => {
            setisLoading(true)
            dispatch(pendingPostAction.getPendingUnAssignedStoriesList('pendingPosts', pendingPostList?.nextPage, setAlert, postFilter, () =>  setisLoading(false)));
            dispatch({ type: SELECTED_PENDING_POSTS, payload: [] });
            handleScroll();
          }}
